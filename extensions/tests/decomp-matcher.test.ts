...
test("runs an allowlisted matcher child, validates its completion event, and rechecks the candidate", async () => {
	...
	await writeFile(scratch, `#!${process.execPath}\nimport { mkdir, readFile, writeFile } from 'node:fs/promises'; await mkdir(process.argv[3], {recursive:true}); if (!(await readFile(process.argv[2],'utf8')).includes('return 1')) process.exit(1); await writeFile(process.argv[3]+'/ok','ok'); console.log('matched 4/4'); console.error('scratch diagnostic');\n`);
	...
	await withEnv({ PI_LAUNCHER_BIN: launcher, PI_DECOMP_MATCHER_CONFIG: JSON.stringify({ version: 1, jobRoot: path.join(root, ".jobs"), default: { provider: "fixture", model: "model", reasoning: "low" }, allowed: [{ provider: "fixture", model: "model", reasoning: "low" }] }) }, async () => {
		...
		const manifest = JSON.parse(await readFile(path.join(result.details.jobDirectory, "manifest.json"), "utf8"));
		assert.equal(result.details.session, manifest.paths.sessionDirectory);
		assert.equal(result.details.baseline.attempt, 0); assert.equal(result.details.baseline.exitCode, 0);
		assert.match(result.details.baseline.candidateSha256, /^[0-9a-f]{64}$/); assert.equal(result.details.baseline.artifactDirectory, manifest.paths.baselineDirectory);
		assert.equal(result.details.baseline.candidatePath, manifest.paths.baselineCandidate); assert.notEqual(result.details.baseline.candidatePath, manifest.paths.bestCandidate);
		assert.equal(await readFile(result.details.baseline.candidatePath, "utf8"), "int f(void) { return 1; }\n");
		assert.equal(result.details.baseline.stdout, "matched 4/4\n"); assert.equal(result.details.baseline.stderr, "scratch diagnostic\n");
		assert.equal(await readFile(result.details.baseline.stdoutPath, "utf8"), result.details.baseline.stdout);
		assert.equal(await readFile(path.join(result.details.baseline.artifactDirectory, "ok"), "utf8"), "ok");
